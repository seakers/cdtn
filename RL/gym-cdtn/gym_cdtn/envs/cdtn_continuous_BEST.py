...
class CdtnEnvContinuousBest(gym.Env):
    ...
    def __init__(self):
        ...
        config_file = './RL/inputs/lunar_scenario_JAIS2021.yaml'
        ...
        self.bits_dropped_45_window = deque(maxlen=2)
        self.bits_dropped_45_window.append(0)
    ...
    def step(self, action):
        ...
        return next_state_vec, reward, is_done, {'reward': reward, 'benefit': benefit,
                                                 ...
                                                 'dropped': np.sum(self.bits_dropped_45_window)}

    def reset(self):
        print("JAIS2021 environment with slightly modified reward function")
        self.bits_mem_window.clear()
        self.bits_mem_window.append(0)
        self.bits_arrived_destination_through_45_window.clear()
        self.bits_arrived_destination_through_45_window.append(0)
        self.bits_dropped_45_window.clear()
        self.bits_dropped_45_window.append(0)
        self.env.reset()
    ...
    def observe_reward(self, state):
        ...
        for bundle in node_45_dropped_queue:
            bits_45_dropped += bundle.data_vol

        # Add the cumulative number of bits arrived from all previous steps and add to queue
        self.bits_dropped_45_window.append(np.sum(self.bits_dropped_45_window))
        # Compute the number of bits arrived in this time step and add to queue
        self.bits_dropped_45_window.append(bits_45_dropped - self.bits_dropped_45_window[-1])

        # Benefit: bits that arrive at destination correctly in current time step
        benefit = self.bits_arrived_destination_through_45_window[-1] - self.bits_dropped_45_window[-1]

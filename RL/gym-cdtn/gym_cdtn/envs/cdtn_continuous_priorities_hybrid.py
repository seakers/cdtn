...
class CdtnEnvContinuousPrioritiesHybrid(gym.Env):
    ...
    def __init__(self):
        ...
        config_file = './RL/gym-cdtn/inputs/lunar_scenario7.yaml'
        ...
        # The action space includes 6 different types of actions that the intelligent
        # agent--i.e., the lunar gateway-- can take depending on the current state of the system:
        # 1. Increasing (doubling) the data rate ($Rb_{in}$) of all links with the neighbor nodes transmitting bundles
        #    to the gateway.
        # 2. Decreasing (by half) the data rate ($Rb_{in}$) of all links with the neighbor nodes transmitting bundles
        #    to the gateway.
        # 3. Increasing (doubling) the data rate ($Rb_{out}$) of the downlinks with the DSN and crosslink.
        # 4. Decreasing (by half) the data rate ($Rb_{out}$) of the downlinks with the DSN and crosslink.
        # 5. Routing bundles through crosslinks instead of sending them straight to the DSN.
        # 6. Do nothing (i.e., not changing any parameter of the network).
        # This approach combines RL and expert rules and the dropping action was eliminated. The intelligent agent is
        # no longer responsible for dropping packets and, Instead, 3 very simple rules were incorporated inside the
        # DTN nodes to take care of selective bundle dropping depending on the level of buffer congestion. See
        # {@DtnNodeRLHybrid} to see how the dropping is taken care of.
        self.action_space = spaces.discrete.Discrete(0 + 2 + 2 + 1 + 1)
        ...
        self.bits_dropped_45_window_total = deque(maxlen=2)
        self.bits_dropped_45_window_total.append(0)
    ...
    def reset(self):
        print("Hybrid priorities environment")
        ...
        self.bits_dropped_45_window_high.append(0)
        self.bits_dropped_45_window_total.clear()
        self.bits_dropped_45_window_total.append(0)
        self.env.reset()
    ...
    def observe_reward(self, state): # like reward8 but keeping track of different dropped priorities
        ...
        # Add the cumulative number of bits arrived from all previous steps and add to queue
        self.bits_dropped_45_window_total.append(np.sum(self.bits_dropped_45_window_total))
        # Compute the number of bits arrived in this time step and add to queue
        self.bits_dropped_45_window_total.append(bits_45_dropped_total - self.bits_dropped_45_window_total[-1])

        # Benefit: bits that arrive at destination correctly in current time step
        benefit = self.bits_arrived_destination_through_45_window[-1] - self.bits_dropped_45_window_total[-1]
